...
function NavOptions({ setWhichFilter, skills, username, setUser, setFilter, setIsSettings, fetchProfiles }) {
    ...
    return(
        <nav className='relative w-full flex justify-between shadow-xl'>
            ...
            <div className={`${isNavDropDown ? 'block' : 'hidden'} absolute sm:contents w-full sm:right-2.5 top-full flex flex-col gap-5 sm:flex-row sm:justify-between items-center bg-black sm:bg-transparent text-white sm:text-black p-0`}>
                {skills?.map((obj, index) => <HeaderButton setWhichFilter={setWhichFilter} key={obj.category} category={obj.category} text={obj.category} canHover={true} isLink={false} setFilter={setFilter} showRight={index > 1 ? true : false} obj={obj} />)}
